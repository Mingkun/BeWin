...
def main():
    ...
    for record in records:
        ...
        if event == 'callback_duplicate_code':
            duplicate_code += 1
        if event in {'authorize_redirect', 'token_request', 'userinfo_request', 'userinfo_attempt'}:
            detail = {
                'event': event,
            ...
            if event == 'userinfo_attempt':
                detail['success'] = '否' if not str(record.get('status_code') or '').startswith('2') else ('否' if (record.get('response_preview') or '').find('error') >= 0 else '是')
            else:
                detail['success'] = '已发起'
